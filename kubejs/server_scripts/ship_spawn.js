...
PlayerEvents.tick(event => {
  ...
  if (vehicle) {
    ...
    if (player.tags.contains('on_ship')) {
      ...
      if (
        ship.name.string.toLowerCase() === ship.type.split(':')[1].toLowerCase()
      ) {
        shipName = [
      ...
      Utils.server.scheduleInTicks(1, callback => {
        Utils.server.runCommandSilent(
          `execute as ${player.name.string} run surveyor landmarks add surveyor:point ${pos.x} ${pos.y} ${pos.z} ${color} ${shipName}`
        )
      })
    }
  }
  ...
  if (shipID) {
    ...
    if (ship) {
      ...
      Utils.server.runCommand(
        `execute as ${event.player.username} run spawnpoint @s ${Math.floor(
          ship.x
        )} ${Math.floor(ship.y) + 1} ${Math.floor(ship.z)}`
      )
    } else {
