ServerEvents.recipes(event => {
    ...
    event.shapeless('minecraft:lead', [
    ...
    event.shapeless('comforts:rope_and_nail', [
        'supplementaries:rope',
    ...
    const MAX_DAMAGE_ROPE_ARROW = Item.of(
    ...
    for (let j = 1; j <= MAX_DAMAGE_ROPE_ARROW; j++) {
        for (let i = 1; i <= Math.min(j, 8); i++) {
            ...
            event.shapeless(repairedRopeArrow, [
                damagedRopeArrow,
                `${i}x supplementaries:rope`,
            ])
        }
    }
