...
ItemEvents.entityInteracted('kubejs:ship_speed_upgrade', event => {
    ...
    if (shipTypes.includes(event.target.type)) {
        ...
        if (speedData["upgradeCount"] < ShipUpgradeConfig["speed"]["upgradeCaps"][ship.type]) {
            ...
            event.item.shrink(1);
            event.level.playSound(null, ship.x, ship.y, ship.z, "supplementaries:block.bellows.blow", "ambient", 1, 1);
            event.server.runCommandSilent(`particle glow ${ship.x} ${ship.y + 2} ${ship.z} 0.001 0.01 0.001 0.01 10`);
        } else {
            event.level.playSound(null, ship.x, ship.y, ship.z, "block.note_block.didgeridoo", "ambient", 1, 1);
        }
        event.cancel();
    }
});
...
ItemEvents.entityInteracted('kubejs:ship_cargo_upgrade', event => {
    ...
    if (shipTypes.includes(event.target.type)) {
        ...
        if (cargoData["upgradeCount"] < ShipUpgradeConfig["cargo"]["upgradeCaps"][ship.type]) {
            ...
            event.server.runCommandSilent(`particle glow ${ship.x} ${ship.y + 2} ${ship.z} 0.001 0.01 0.001 0.01 10`);
            event.level.playSound(null, ship.x, ship.y, ship.z, "block.barrel.close", "ambient", 1, 1);
